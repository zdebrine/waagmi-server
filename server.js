...
const {
    SaveFinancesToDb,
} = require("./client/SaveFinancesToDb/SaveFinancesToDb.ts");
const {
    GetUserInvestments,
...
const formatString = require("./helpers/formatString.js");
...
app.get("/bank_transactions", async (req, res) => {
    ...
    const sixMonthsAgo = now.subtract(6, "months").format("YYYY-MM-DD");

    const lastMonthTransaction = await plaidClient.transactionsGet({
        access_token,
        start_date: thirtyDaysAgo,
        end_date: today,
    });
    const lastSixMonthTransaction = await plaidClient.transactionsGet({
        access_token,
        start_date: sixMonthsAgo,
        end_date: today,
    });
    const transactions = lastMonthTransaction.data.transactions;
    const sixMonthTransactions = lastSixMonthTransaction.data.transactions;
    const companyId = req.query.companyId
    const revenue = findRevenue(transactions)
    const profits = findProfit(transactions)
    const expenses = findExpenses(transactions)
    const formattedTransactions = formatString(JSON.stringify(sixMonthTransactions))
    SaveFinancesToDb(Number(revenue), Number(expenses), Number(profits), formattedTransactions, companyId, pool)
    const responseBody = {
        revenue: revenue,
        profits: profits,
        expenses: expenses,
